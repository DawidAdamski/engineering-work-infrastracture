...
1. **LoadBalancer with MetalLB** - Production-like LoadBalancer services (recommended)
2. **NodePort Services** - Direct access via Minikube node IP and assigned ports
3. **Ingress with Manual nginx-ingress** - HTTP/HTTPS routing via Ingress controller (installed manually, not via addon)

---

## Option 1: LoadBalancer with MetalLB (Recommended)
...
## Option 2: NodePort Services (Simplest Alternative)
...
Use **MetalLB LoadBalancer** (Option 1) for realistic testing:
...
- Works without Minikube addons

### For Quick Development/Testing

Use **NodePort** (Option 2) for simplicity:
